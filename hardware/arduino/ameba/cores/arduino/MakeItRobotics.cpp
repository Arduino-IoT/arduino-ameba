...
IMAGE2_DATA_SECTION
uint32_t current_IR_time;

IMAGE2_DATA_SECTION
uint32_t diff_IR_time;
...
IMAGE2_DATA_SECTION
static unsigned long IR_time[12];

IMAGE2_DATA_SECTION
static int test_pos=0;

IMAGE2_DATA_SECTION
static int test_ir_bits = 0;
...
void MakeItRobotics::remote_scan(void)
{
	...
	u8 pin_num = 3; // pin 8 -> pin_num=3 ; gpio_get_pin_num(8);
  ...
  RegValue =  HAL_READ32(GPIO_REG_BASE, GPIO_INT_POLARITY);
  RegValue &= ~(1 << pin_num);	  
  RegValue |= (polarity << pin_num);
  HAL_WRITE32(GPIO_REG_BASE, GPIO_INT_POLARITY, RegValue);
  polarity = !polarity;
}
