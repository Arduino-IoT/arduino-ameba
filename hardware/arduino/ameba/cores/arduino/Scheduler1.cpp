...
{
	SchedulerTask sub_task;

	sub_task = (SchedulerTask)args;
	if ( sub_task == NULL ) {
		DiagPrintf("loop_thread: sub_task == NULL \n");
		return;
	}
...
SchedulerClass1::SchedulerClass1() 
{
}


void SchedulerClass1::startLoop(SchedulerTask task, uint32_t stackSize) 
{	
	Thread* tid;
	tid = new Thread(loop_thread, (void*)task, osPriorityNormal, stackSize);
	tid->start();
}
